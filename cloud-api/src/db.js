...
const stmt = {
  novoEnsaio: db.prepare(`INSERT INTO ensaios (device, diam_mm) VALUES (?, ?)`),
  addLeitura: db.prepare(`INSERT INTO leituras (ensaio_id, t_ms, carga_kN, tensao_MPa, evento) VALUES (?,?,?,?,?)`),
  finalizar: db.prepare(`UPDATE ensaios SET status='finalizado', pico_kN=?, tensao_MPa=?, finished_at=datetime('now') WHERE id=?`),
...
export function novoEnsaio(device, diam_mm = 100) {
  return stmt.novoEnsaio.run(device, diam_mm).lastInsertRowid;
}
...
export function finalizarEnsaio(ensaioId, pico_kN, tensao_MPa) {
  stmt.finalizar.run(pico_kN, tensao_MPa, ensaioId);
}
